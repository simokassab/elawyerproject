...
$('#container').highcharts({
        ...
        title: {
            text: 'القضايا الخاسرة لكل محامي'
        },
		credits: {
			 text: 'Techoffice.co',
...
mysqli_query($con, "SET NAMES 'utf8'");
mysqli_query($con, 'SET CHARACTER SET utf8');
$query="SELECT cases.lawyer_id as lawyerid , count(cases.ID) as countt from users, cases where users.ID=cases.lawyer_id and cases.status='LOOSE' group by lawyerid";
$result = mysqli_query($con, $query);
while ($rows = mysqli_fetch_assoc($result)) {
	$content .= "{name: '".GetFnLnByID($rows['lawyerid'])."',
y: ".$rows['countt']." },";
}
