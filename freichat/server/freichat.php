...
//----------------------------------------------------------------------
    public function get_messages($time) {
        ...
        if ($time == 0) {
            ...
            $get_mesg_query = "SELECT * FROM frei_chat WHERE (frei_chat.\"to\"=" . $frm_id . " OR (frei_chat.\"from\"=" . $frm_id . " AND message_type<>3)) 
                            AND message_type<>1 AND time>:time order by time";
        ...
        foreach ($result as $res) {
            ...
            if (!isset($_SESSION[$this->uid . 'can_receive_video_messages']) && $res['message_type'] == 2) {
...
//-------------------------------------------------------------------
    public function post() {
        ...
        if (in_array($_POST['message_type'], array(0, 2, 3, 4))) {
        ...
        foreach ($messages as $message) {
            ...
            $time = time() . str_replace(" ", "", microtime());
            if (in_array($message_type, array(0, 2, 3, 4))) {
...
//--------------------------------------------------------------------------
    public function getdata() {
        ...
        $getdata_query = "SELECT * FROM frei_chat WHERE (frei_chat.\"to\"=" . $frm_id . " OR (frei_chat.\"from\"=" . $frm_id . " AND message_type<>3)) 
                            AND message_type<>1 AND message_type<>2 AND message_type<>4 order by time limit 200";
        ...
        foreach ($messages as $res) {
            ...
            //if(!isset($arr[$res['from']])) $arr[$res['from']] = array();
            $arr[$i] = $res;
            ...
            if (!in_array($res['from'], $allowed_ids)) {

                $allowed_ids[] = $res['from'];
                $exist = true;
            }
        ...
        $freichat->messages = $mesgs;
        $freichat->forced = $forced;
    ...
    public function isset_mesg() {
        ...
        if (isset($_GET['mobile']) && $_GET['mode'] == "chatroom") {
            ...
            $isset_mesg_query = "SELECT * FROM frei_chat WHERE ((frei_chat.\"to\"=" . $frm_id . " AND frei_chat.\"from\"=" . $this->db->quote($to_id) . ")
                                    ...
                                    AND message_type <> 2 AND message_type <> 3 AND message_type <> 4 order by time";
