...
//------------------------------------------------------------------------------
     ...
     *  The generic method written is slow and only searches for online users
     *  so a specific query must be written for every driver in the future
     */
    public function search_users($keyword = '') {
        ...
        foreach($users as $user) {
            ...
            if(strpos($user['username'], $keyword) === false) continue;
    ...
    public function get_guests() {
        ...
        $query = "SELECT DISTINCT status_mesg,username,session_id,status,guest,in_room
                  ...
                  WHERE time>" . $this->online_time2 . "
                   ...
                   AND status<>2
                   AND status<>0 ORDER BY username ASC";
//echo $query;
