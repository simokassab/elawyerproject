...
$date = date('Y-m-d H:i:s');
    ...
    if(isset($_GET["sub"])) {
        ...
        $id = NewForm(0, $casedate,  $customer, $customer2, $customer3, $customer4, $IDNUMBER, $t1, $t2, $t3, $t4, $t5, $t6,
            ...
            $id = NewForm($customerid, $casedate,  '', '', '', '', 0, '', '', '', '', '', '',
                $cust_desc, 0, $opponent, $opponent2, $opponent3, $opponent4, $opt1, $opt2, $opt3, $OPIDNUMBER, $opponent_desc, $subject, $ctype,
                nl2br($details), $price, $paid, $remaining, $paidtype, $lawyer, $consultant, $assigned, trim(nl2br($comments)), 'PENDING');
        }
        NewNotification($lawyer, $audit, "index.php?action=NEWFile/request&id=".$id."&from=".$lawyer, "رسالة من المحامي ".GetFnameLnameByUserID($lawyer), 'NOTREAD' );
        NewSuperEvent("تم إرسال الملف إلى قسم التدقيق", $lawyer );
        header("location: ../index.php");
    }
if(isset($_GET['act']) &&( $_GET['act']=='check')){
    if(CheckIfOpponent($_GET['c'], $_GET['c2'], $_GET['c3'], $_GET['c4'])){
        $res= "1";
        UpdateStatusForm($_GET['idd'], "REFUSED");
    }
    ...
    if($res==""){
        UpdateStatusForm($_GET['idd'], "ACCEPTED");
    }
    echo $res;
}
if(isset($_GET['act']) &&( $_GET['act']=='sendToLawyer')){

    NewNotification($_GET['from'],$_GET['lawyer'], 'index.php?action=NEWFile/request&act=sendToLawyer&id='.$_GET['idd'].'&res='.$_GET['res'],'رسالة من قسم التدقيق '.GetFnameLnameByUserID($lawyer),'NOTREAD');
    NewSuperEvent("قسم التدقيق أرسل الملف إلى المحامي", 5 );
}
if (isset($_GET['act']) &&($_GET['act']=="opencase")){
   ...
   // echo $assign;
    ...
    $case_id=NewCase($customer_id, $cust_desc, $casedate,$ctype,$lawyer,$consultant,$opponent_id, $opponent_desc,
        $price,$subject,$details ,'PENDING');
    NewSuperEvent("المحامي كم بفتح القضية: ".$subject, $lawyer );
    
    $contract=NewContract($account, $paidtype, $case_id);
    NewAccountAction($contract, $paid, $remaining, 'اتعاب العقد'   ,$casedate, 0,0,0, '');
    makeDir("../elfinder/files/".$_SESSION['office'].'/'.$lawyer."/".$case_id."_".replace_($subject, ' ', '_'));
    if($customerid==0)
        echo $case_id;
    else {
...
if (isset($_GET['updatee'])){
    ...
    header("location: ../index.php?action=cases&id_case=".$_GET['id_case']);
}
ob_end_flush();
?>